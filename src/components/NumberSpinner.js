...
function NumberSpinner({ id: idProp, label, error, size = 'medium', ...other }) {
  ...
  return (
    <BaseNumberField.Root sx={{'& .MuiBox-root':{
      ...
      render={(props, state) => (
        <FormControl
          ...
          sx={{
            '& div.MuiBox-root': {
                border: "1px gold solid",
                width: "clamp(50px,9vw,250px)"
            },
            '& .MuiButtonBase-root':{width:"clamp(15px,3vw,45px)",padding: "0px !important"},
    ...
    >
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        >
          <RemoveIcon fontSize="clamp(5px, 3vw, 20px)" />
        </BaseNumberField.Decrement>
        ...
        >
          <AddIcon fontSize="clamp(5px, 3vw, 20px)" />
        </BaseNumberField.Increment>
      </Box>
